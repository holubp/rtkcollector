...
| In-phone RTKLIB real-time solution | `2026-06-06-rtkcollector-v1-workflows.md`, `2026-06-19-rtklib-ex-realtime-mvp.md` | In progress | V1 deliberately leaves RTKLIB real-time disabled. RTKLIB-EX routing/API foundation, app-side RTKLIB profiles/settings references, separate RTKLIB session artifacts, local pinned-checkout automation and UM980 sample-readiness tooling exist. Native RTKLIB-EX source/JNI build, recording-service fanout, live dashboard card and hardware replay validation remain open. The plan requires forward-only native-library integration, explicit input routing and strict separation from receiver-internal solutions. |
