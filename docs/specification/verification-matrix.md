...
| `SESSION-RTKLIB-001` | Automated + manual | RTKLIB output writer tests; `tools/rtklib_replay_session_check.py`; RTKLIB-enabled session later | In progress | NMEA/POS artifact model and native output bridge exist. The session checker verifies that enabled RTKLIB sessions with status solutions also contain NMEA/POS solution rows. Host native build and fresh field/replay validation remain open. |
