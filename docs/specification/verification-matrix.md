...
| `SESSION-RTKLIB-001` | Automated + manual | RTKLIB output writer tests; RTKLIB synthetic-output tests; `SessionNmeaExporterTest`; `tools/rtklib_replay_session_check.py`; RTKLIB-enabled session later | In progress | NMEA/POS artifact model and native output bridge exist. If native text output is missing while a valid snapshot exists, Kotlin synthesizes minimal RTKLIB NMEA/POS lines. Source-aware NMEA sharing keeps RTKLIB NMEA separate from receiver NMEA. Host native build and fresh field/replay validation remain open. |
