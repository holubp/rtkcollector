...
| `WF-RTKLIB-001` | Automated + review | RTKLIB input route tests; V1 code review | In progress | Routing/API foundation is passing; native RTKLIB-EX engine and app activation remain open. |
...
| `RX-UM980-RTKLIB-001` | Automated + review | RTKLIB input route tests | Passing | OBSVMB is direct; OBSVMCMPB requires converter/direct decoder declaration. |
...
| `SESSION-RTKLIB-001` | Automated + manual | RTKLIB output writer tests; RTKLIB-enabled session later | In progress | NMEA/POS artifact model exists; native engine output remains open. |
