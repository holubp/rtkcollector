...
| In-phone RTKLIB real-time solution | `2026-06-06-rtkcollector-v1-workflows.md`, `2026-06-19-rtklib-ex-realtime-mvp.md` | In progress | V1 deliberately leaves RTKLIB real-time disabled. RTKLIB-EX routing/API foundation, app-side RTKLIB profiles/settings references and separate RTKLIB session artifacts exist; native RTKLIB-EX snapshot, JNI bridge, recording-service fanout, live dashboard card and hardware replay validation remain open. The plan requires forward-only native-library integration, explicit input routing and strict separation from receiver-internal solutions. |
