...
| `WF-RTKLIB-001` | Automated + review | RTKLIB input route tests; RTKLIB server-backend source-structure tests; RTKLIB profile frequency-count tests; M8T field replay using `samples/debug/session-2026-06-21T17-08-13...zip`; Android Studio/CI native `.so` build | In progress | Routing/API foundation, bounded worker and JNI/native bridge are implemented. Current native bridge is being replaced with RTKLIB server-style memory-buffer processing. Native `.so` build still needs validation on a host with a working Android NDK. |
