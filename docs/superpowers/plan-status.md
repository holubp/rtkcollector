...
| u-blox M8T support and Android mock provider | `2026-06-13-ublox-m8t-and-mock-provider.md`, `2026-06-16-pr-1-ublox-mock-provider-integration.md`, `2026-06-22-rtklib-card-and-ublox-dop-monitoring.md`, `2026-06-22-mock-provider-api-maximization.md` | Implemented, not field-tested | UBX framing/parser/profile code, NAV-PVT/NAV-SAT/NAV-DOP monitoring, solution arbitration and mock-location adapter exist with tests. Mock altitude uses ellipsoidal height, and the adapter now publishes available vertical accuracy and MSL altitude through API-gated Android `Location` fields. Satellite counts are exposed as best-effort extras; full `GnssStatus` remains unavailable through Android's public mock-location API. Needs M8T hardware validation and API 26+/34+ mock-provider field checks before marking done. |
...
| In-phone RTKLIB real-time solution | `2026-06-06-rtkcollector-v1-workflows.md`, `2026-06-19-rtklib-ex-realtime-mvp.md`, `2026-06-19-rtklib-ex-native-and-obsvmcmpb-completion.md`, `2026-06-21-rtklib-rtksvr-native-backend.md`, `2026-06-22-rtklib-card-and-ublox-dop-monitoring.md` | In progress | RTKLIB-EX routing/API foundation, bounded worker, app-side profiles/settings references, UM980 OBSVMB RTKLIB command profile, separate session artifacts, route/snapshot metadata, local pinned-checkout automation, native JNI bridge/build glue, NMEA/POS native output plumbing, named OBSVMCMPB decoder shim, UM980 sample-readiness tooling and dashboard RTKLIB card with solution lat/lon/ellipsoidal-height/H-V accuracy are implemented. The JNI bridge now uses RTKLIB `rtksvr_t` memory-buffer stream processing instead of a single latest RTCM observation snapshot. Local Termux has no valid runnable NDK for proving the native `.so`; Android Studio/CI native build and real replay/field validation remain required. Current local UM980 samples contain monitoring/correction frames but no actual OBSVMB/OBSVMCMPB observation frames, so OBSVMCMPB solution validation remains open. Current M8T baseline evidence from the old bridge remains `quality 4:1274` and `satellites 3:1274`; a new APK field replay is required to validate the server backend improvement. |
