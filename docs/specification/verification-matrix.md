...
| `SESSION-RTKLIB-001` | Automated + manual | RTKLIB output writer tests; RTKLIB synthetic-output tests; `tools/rtklib_replay_session_check.py`; RTKLIB-enabled session later | In progress | NMEA/POS artifact model and native output bridge exist. If native text output is missing while a valid snapshot exists, Kotlin synthesizes minimal RTKLIB NMEA/POS lines. The session checker verifies that enabled RTKLIB sessions with status solutions also contain NMEA/POS solution rows. Host native build and fresh field/replay validation remain open. |
